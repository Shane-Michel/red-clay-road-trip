...
final class ShareToken
{
    ...
    public static function create(int $tripId, ?string $ttl = null): array
    {
        $expires = self::calculateExpiry($ttl);
        $signature = self::sign($tripId, $expires);
    ...
    public static function validate(int $tripId, int $expires, string $signature): bool
    {
        if ($expires <= 0) {
            return false;
        }
        ...
        $expected = self::sign($tripId, (new \DateTimeImmutable('@' . $expires))->setTimezone(new \DateTimeZone('UTC')));
    ...
    private static function sign(int $tripId, \DateTimeImmutable $expires): string
    {
        $payload = $tripId . '|' . $expires->getTimestamp();
